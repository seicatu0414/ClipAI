    ...
    def _parse(raw: str, chunk: TranscriptChunk) -> list[KnowledgeObservation]:
        try:
            ...
            LOGGER.warning(
                ...
                extra={"reason": "LLM response is not valid JSON"},
            )
            return []
        ...
        if not isinstance(items, list):
            LOGGER.warning(
                "knowledge_response_rejected",
                extra={"reason": "LLM response must contain an observations list"},
            )
            return []
        ...
        for item in items:
            try:
                observations.append(_parse_observation(item, chunk, segment_map))
            except (TypeError, ValueError) as error:
                LOGGER.warning(
                    "knowledge_observation_rejected",
...
def _parse_observation(
    ...
    segment_map: dict[int, TranscriptSegment],
) -> KnowledgeObservation:
